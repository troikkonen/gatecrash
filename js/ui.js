...
function uiGameOver(extra){ $('overS').textContent = 'Level ' + G.level + (extra ? '  ·  ' + extra : ''); $('over').style.display = 'flex'; }
...
function openPicker(){
  ...
  for (let L=1; L<=TOTAL_LEVELS; L++){ const b = document.createElement('button'); b.innerHTML = L + (SAVE.stars[L] ? '<s>' + '★'.repeat(SAVE.stars[L]) + '</s>' : '');
    if (L > SAVE.level) b.className = 'locked'; else b.onclick = () => { audioInit(); for (const p of ['title','win','over','picker']) $(p).style.display = 'none'; G.paused = false; lastT = performance.now(); startLevel(L); };
    box.appendChild(b); }
...
$('playBtn').onclick = () => { audioInit(); $('title').style.display = 'none'; G.paused = false; lastT = performance.now(); startLevel(G.level); };
$('lvl').onclick = openPicker;
